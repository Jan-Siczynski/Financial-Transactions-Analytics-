...
-- Q10: What percentage of annual income did the TOP 10 customers spend
--      on transactions in 2010?
--      Identifies highest-spending customers relative to their income
--      as a baseline for financial health assessment.
...
SELECT
    CASE EXTRACT(MONTH FROM date::date)
        ...
        WHEN 12 THEN 'Dec'
    END AS month,
    COUNT(amount) AS volume
FROM transactions_data
GROUP BY EXTRACT(MONTH FROM date::date), month
...
SELECT
    ...
    SUM(CASE WHEN f.fraud = 'Yes' THEN 1 ELSE 0 END) AS fraud_count,
    ROUND(SUM(CASE WHEN f.fraud = 'Yes' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS fraud_rate_pct
...
SELECT
    CASE
        ...
        ELSE 'Excellent'
    END AS credit_segment,
    ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) AS fraud_rate_pct
FROM transactions_data AS t
    INNER JOIN fraud_labels AS f ON t.id = f.transaction_id
    INNER JOIN users_data AS u ON u.id = t.client_id
...
WITH retirement_segments AS (
    SELECT
        ...
        CASE
            WHEN (retirement_age - current_age) <= 5 THEN 'Near retirement (≤ 5 years)'
            WHEN (retirement_age - current_age) BETWEEN 6 AND 15 THEN 'Mid career (6-15 years)'
            WHEN (retirement_age - current_age) BETWEEN 16 AND 30 THEN 'Early career (16-30 years)'
            ELSE 'Young (> 30 years)'
        END AS career_stage
    FROM users_data
)
...
income_ratio AS (
    ...
    WHERE REPLACE(u.yearly_income, '$', '')::numeric > 300
)
SELECT
    *,
